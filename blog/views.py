...
def photo_upload(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # set the uploader to the user before save the model
...
def blog_and_photo_upload(request):
    blog_form = forms.BlogFrom()
    photo_form = forms.PhotoForm()
    if request.method == 'POST':
        # handle POST request here
        blog_form = forms.BlogFrom(request.POST)
...
def edit_blog(request, blog_id):
    blog = get_object_or_404(models.Blog, id=blog_id)
    edit_form = forms.BlogFrom(instance=blog)
    delete_form = forms.DeleteBlogForm()
    if request.method == 'POST':
        if 'edit_blog' in request.POST:
            edit_form = forms.BlogFrom(request.POST, instance=blog)
